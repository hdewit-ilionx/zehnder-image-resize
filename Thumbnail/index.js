...
module.exports = (context, eventGridEvent, inputBlob) => {
  ...
  context.log("Blob uploaded to " + blobUrl);
  context.log("Container with uploaded images = " + containerNameUpload);
...
function uploadResizedCopy(thumbnail, context, widthInPixels, containerName, doResize) {
  ...
  const fileExtension = blobUrl.slice(blobUrl.lastIndexOf("."));
  const fileName = getArticleCode(containerName, blobName) + fileExtension;

  context.log("upload resized " + fileName + " to container " + containerName);

  if (doResize) { thumbnail.resize(widthInPixels, Jimp.AUTO) };
  ...
  function getArticleCode(containerName, blobName) {
    ...
    blobService.getBlobMetadata(containerName, blobName, function (error, result) {
      ...
      return result.metadata.article_code;
